...
public class CpuMemMonitor extends AbstractMonitor{
	...
	private static final String[] cmds = {"top", "-m", "5"};
	...
	public void startMonitor() {
		String fileName = createFileName("MonitorInfo", mPkgName);
	...
	private final void doMonitorLoop(String fileFullPath) {
		...
		Config.MonitorInfo = fileFullPath;
		...
		while (!mStopMonitor) {
			try {
				...
				try {
					TimeUnit.MILLISECONDS.sleep(Integer.parseInt(MonitorType.COLLECT_FREQUENCY_CPUMEM));
	...
	private final String makeOutputStyle(String mmUseage,String cpuUseage){
		return "Mem|Cpu:" + mmUseage + "|" + cpuUseage;
	}
