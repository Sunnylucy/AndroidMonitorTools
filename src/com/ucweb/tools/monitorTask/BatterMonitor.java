...
public class BatterMonitor extends AbstractMonitor{
	...
	public BatterMonitor(Context context, String fileSavePath, String pkgName){
		...
		mFilefullPath = createFileFullPath(fileSavePath, createFileName("BatterInfo", pkgName));
		Config.BatteryInfo = mFilefullPath;
			...
			if (Intent.ACTION_BATTERY_CHANGED.equals(intent.getAction())) {
				...
				executor.execute(new Runnable() {
					...
					public void run() {
						...
						String batter = String.format(Locale.CHINA, "%.1f", current*(100.0) / total);
						//get batter temperature
						String temperature = cTemperatureData(intent.getIntExtra(BatteryManager.EXTRA_TEMPERATURE, 0));				
	...
	private final String cTemperatureData(int temperature){
		if(temperature > 100){
